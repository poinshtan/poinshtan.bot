...
local function info_cb(arg, data)
  ...
  lastname = ""
   ...
   local text = "_First name :_ *"..firstname.."*\n_Last name :_ *"..lastname.."*\n_Username :_ "..username.."\n_ID :_ *"..data.id_.."*\n\n"
		    if data.id_ == tonumber(Solid) then
		       text = text..'_رتبه :_ *خالق ربات 😍*\n\n'
			   elseif is_sudo1(data.id_) then
	           text = text..'_رتبه :_ *ادمین با دسترسی کامل❤️*\n\n'
		     elseif is_admin1(data.id_) then
		       text = text..'_رتبه :_ *ادمین ربات😊*\n\n'
		     elseif is_owner1(arg.chat_id, data.id_) then
		       text = text..'_رتبه :_ *مالک گروه🤗*\n\n'
		     elseif is_mod1(arg.chat_id, data.id_) then
		       text = text..'_رتبه :_ *ادمین گروه 🤓*\n\n'
		 else
		       text = text..'_رتبه :_ *ممبر معمولی 😐*\n\n'
			end
         local user_info = {} 
  ...
  text = text..'تعداد پیامها : '..user_info_msgs..'\n\n'
  text = text..'#Poinshtan'
...
local function info2_cb(arg, data)
  ...
  lastname = ""
   ...
   local text = "_First name :_ *"..firstname.."*\n_Last name :_ *"..lastname.."*\n_Username :_ "..username.."\n_ID :_ *"..data.id_.."*\n\n"
		    if data.id_ == tonumber(Solid) then
		       text = text..'_Rank :_ *خالق ربات 😍*\n\n'
			   elseif is_sudo1(data.id_) then
	           text = text..'_Rank :_ *ادمین با دسترسی کامل❤️*\n\n'
		     elseif is_admin1(data.id_) then
		       text = text..'_Rank :_ *ادمین ربات😊*\n\n'
		     elseif is_owner1(arg.chat_id, data.id_) then
		       text = text..'_Rank :_ *مالک گروه🤗*\n\n'
		     elseif is_mod1(arg.chat_id, data.id_) then
		       text = text..'_Rank :_ *ادمین گروه 🤓*\n\n'
		 else
		       text = text..'_Rank :_ *ممبر معمولی 😐*\n\n'
		 end
         local user_info = {} 
  local uhash = 'user:'..data.id_
  local user = redis:hgetall(uhash)
